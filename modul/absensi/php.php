                      ...
                      <div class="card">
                        <div class="card-body">
                          ...
                          <?php
                            ...
                            FROM jadwal j LEFT JOIN mahasiswa m ON j.id_kelas = m.id_kelas LEFT JOIN matkul mk ON j.kode_mk = mk.kode_mk WHERE m.nim = ? AND j.hari = '$hari_sekarang'  AND j.jam_mulai <= NOW() AND j.jam_selesai >= NOW()";
                            ...
                            // Menampilkan jadwal perkuliahan saat ini dan formulir kehadiran
                            while ($row = $hasilJadwal->fetch_assoc()) {
                                // Pemeriksaan status kehadiran berdasarkan data di tabel kehadiran
                                $idJadwal = $row["id"];
                            ...
                            if ($hasilJadwal->num_rows > 0) {
                                echo "<div class='status-radio'>
                                          ...
                                          <div class='submit-button-container'>
                                              <button class='submit-button' name='submit'>Kirim</button>
                                          </div>
                                      </div></form>";
                            }
                                }}
                            ...
                            if (isset($_POST['submit'])) {
                                ...
                                $kehadiran = "INSERT INTO kehadiran (id_mhs, id_jadwal, tanggal, keterangan) VALUES (?, ?, ?, ?)";
                                $statementKehadiran = $db->prepare($kehadiran);
                                $statementKehadiran->bind_param("ssss", $idMahasiswa, $idJadwal, $tanggal, $keterangan);
